...
The scripted suite (`node scripts/test-all.mjs`, 80 files) proves the parts against a mock
...
node scripts/test-all.mjs                       # 80/80 before you start; a red suite invalidates the walk
...
### Verification
- `node scripts/test-all.mjs` → 80/80 files (§8.1–8.11).
