...
The scripted suite (`node scripts/test-all.mjs`, 85 files) proves the parts against a mock
...
node scripts/test-all.mjs                       # 85/85 before you start; a red suite invalidates the walk
...
### Verification
- `node scripts/test-all.mjs` → 85/85 files (§8.1–8.11).
