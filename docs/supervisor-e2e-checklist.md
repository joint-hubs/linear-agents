...
The scripted suite (`node scripts/test-all.mjs`, 77 files) proves the parts against a mock
...
node scripts/test-all.mjs                       # 77/77 before you start; a red suite invalidates the walk
...
### Verification
- `node scripts/test-all.mjs` → 77/77 files (§8.1–8.11).
