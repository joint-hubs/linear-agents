...
The scripted suite (`node scripts/test-all.mjs`, 74 files) proves the parts against a mock
...
node scripts/test-all.mjs                       # 74/74 before you start; a red suite invalidates the walk
...
### Verification
- `node scripts/test-all.mjs` → 74/74 files (§8.1–8.11).
