...
- **FOC-576 krok 1 (honesty contract) LANDED `3f720e2`** i **PUSHED** (`e2f76f7..3f720e2`, 53 commity — main == origin/main). Komentarz środowiskowy FOC-576 (id `81475b55`): timeline SAC + znany problem maszyny z `docker pull`.
- **FOC-576 krok 2 — kandydat `3d3b779` na `foc-576-dev`** (`9af6247` feat + `3d3b779` docs; baza `7499672`; 3 pliki, +281/−16): `LA_SEMGREP_CMD` = **opt-in override only**, domyślnie goły natywny `semgrep`, **zero fallbacku**; pusty/whitespace = fail-closed (`status:'error'` + exit 2); kontrakt uczciwości kroku 1 nietknięty na obu ścieżkach. Testy **81/81** (stubbed + live-native), żaden nie wymaga dockera. LINT 0 naruszeń, EGRESS exit 0. W toku: REVIEW (`review-10`) → TEST → merge → Done.
- **Ścieżka Docker: „live verification pending — NOT verified"** (dokładne słowa Mateusza) — w `docs/tools/security-scan.md` (:88–105) i w komentarzu kodu (`scripts/security-scan.mjs:277–279`). **Nie jest zweryfikowana i nie wolno jej prezentować jako działającej.** Powód (znany problem maszyny, opisany w FOC-576): wewnętrzny proxy Docker Desktop `http.docker.internal:3128` ucina pobieranie blobów obrazu (`httpReadSeeker: failed open ... EOF`, 13 prób), gdy host pobiera te same URL-e poprawnie (HTTP 206). **Dyrektywy Mateusza: NIE ruszać konfiguracji proxy DD, NIE restartować Dockera (restart zabija kontenery thoughtmap z innej sesji), NIE pobierać nowych binarek z internetu (SAC zablokuje niepodpisane exe tak samo jak semgrep).** Ewentualną zmianę konfiguracji DD zrobi sam.
- **Domyślna ścieżka = natywny semgrep — to wchodzi na main.** `LA_SEMGREP_CMD` tylko opt-in (dokumentowany przepis `docker run --rm -v <root>:/src:ro -w /src semgrep/semgrep:1.178.0 semgrep …`), bez implicit fallbacku. Obraz `semgrep/semgrep:1.178.0` zaakceptowany przez Mateusza, ale **nigdy nie pobrany** (problem pull powyżej).
- **PROBE SAC/semgrep godz. 17:00–17:06 (2026-09-25) — ZIELONY: semgrep skanuje** (re-probe 18:53 i 21:53 wciąż **81/0** — okno otwarte). `security-scan.test.mjs` w foreground w root checkoutu: **81 passed / 0 failed** (baza przy blokadzie SAC: 26/13) + realny skan `security-scan.mjs`: `PASS secretlint (13.0.5)`, `PASS semgrep (1.172.0)`, 0 findings / 574 pliki, exit 0. Odnotowane komentarzem w FOC-576 (id `f7508702`, godzina podniesienia blokady ~12:23). **Warunek 4 przed pushem odblokowany** — push 3 commitów na main (`9af6247`, `3d3b779`, `9705fbf`) czeka na zgodę Mateusza (nie pushuję sam).
- Po FOC-576 Done: tor A — FOC-515 (commit §3.12 na `foc-515-plan`; plan.gate1 jeśli sekcja niezatwierdzona), w oczekiwaniu na Mateusza **FOC-522** (ucinanie tury). Cron `554b74c1` (co ~3 h) odświeża probe `security-scan.test.mjs` — jeśli SAC znów zablokuje, odnotować godzinę w FOC-576.
- **FOC-522 (dev-12) — bramka `gate-dev-12-1` PENDING od 14:56:** diagnoza gotowa (reaper = teardown tury `claude -p`, sufit `CLAUDE_CODE_PRINT_BG_WAIT_CEILING_MS` 600 s; nasze skrypty nie reapują) i 3 kształty A/B/C do wyboru — pytanie przekazane Mateuszowi.

## 2026-09-25 — FOC-545 DONE (`e07a736`) + FOC-576 następny (run 8b24 przejęty)

**DOMKNIĘCIE FOC-545 (decyzja Mateusza: merge przy 84/85 z warunkami 1–5):** werdykt REVIEW `pass` zapisany z mapowaniem AC→dowód; `supervisor-merge` **ACCEPT próba 1** (blast-radius 4 plików testowych zielony w worktree kandydata i w drzewie łączonym; replay 1 commita bez konfliktów); lądowanie **`e07a736`** no-ff na main (baza `23867d2`, 58+/10−). Komentarz FOC-545 (id `f3b5f9a8`) z formułą **„SAST NOT SCANNED (SAC blokuje semgrep)"** + tabela pomiaru after → FOC-545 **Done**. FOC-547 komentarz (lista spawnów bez `LA_STATE_ROOT` = **brak**), 4 nity → FOC-461, FOC-576 komentarz środowiskowy (hashy, timeline, ustalony zakres). **Push wstrzymany** do warunku 4: semgrep realnie skanuje main (natywnie po zejściu SAC albo przez Dockera) + `security-scan.test.mjs` zielony. Worktowey FOC-545 (dev-4/review-5/test-6) żyją — sprzątanie (`supervisor-cleanup`) dopiero za jego zgodą.

**Tor wg decyzji:** FOC-576 (honesty fix: pusty stdout / exit ≠ 0 / nieznana wersja semgrep → `status:'error'` + exit 2, nigdy clean) → tor A: FOC-515 (commit §3.12 na `foc-515-plan`; plan.gate1 jeśli sekcja niezatwierdzona), w oczekiwaniu na jego odpowiedź FOC-522 (ucinanie tury) → tor A 516→517→520→519→476→477 / tor B 381→547→511→512→407→271. Docker-obejście SAC (`LA_SEMGREP_CMD`, `docker run semgrep/semgrep`, repo tylko do odczytu, te same reguły) = drugi krok FOC-576; **obraz semgrep pobieram dopiero po podaniu mu nazwy i rozmiaru do akceptacji**. Cykliczny probe `security-scan.test.mjs` co ~3 h (cron w sesji).

## 2026-09-25 — FOC-545 w toku (run 8b24 przejęty; DEV+REVIEW+TEST domknięte, MERGE ZABLOKOWANY)

Run `2026-09-23T17-08-55-168-supervisor-8b24` (przejęty po padniętej sesji; nowy run NIE zakładany). Kandydat: **`60fe135`** na `foc-545-dev` (baza `23867d2`) — `LA_STATE_ROOT` seam w `scripts/telemetry-server.mjs` (+14/−8) + izolacja scan root w `scripts/rewards-routes.test.mjs` (+44/−2).

- **DEV (dev-4, resume #2, jedna tura foreground):** kontrakt a–e domknięty. `:243 withManifestConsolePid` zamknięte szwem; 6. zmiana w `renderedHistoryRunIds` zostaje; `:532` nietknięte. Standalone `rewards-routes.test.mjs` **10/10 w 1.0 s** (było 9/9 w 11 m 10 s; na main test 1 padał na undici 300 s). Pomiar after (metoda diagnozy): healthy **139 s → 145 ms**, fresh GET **68 250 ms → 13 ms**, cached **>60 s → 6 ms**; produkcyjny fresh **6 204 → 14 201 ms** = głód pętli zdarzeń → **FOC-547** (ścieżka produkcyjna bajtowo identyczna). Lista testów bez `LA_STATE_ROOT`: **brak** (tylko `rewards-routes.test.mjs` spawnuje serwer).
- **REVIEW (review-5): PASS**, 0 blokerów, 4 nity (liczniki diffu +14/−8 vs +15/−7; numery linii; fraza "all run-state reads" za szeroka — `verdict-evidence.mjs:56-58`, `delegation-outcomes.mjs:45-46`, `telemetry-server.mjs:1576` zostają host-bound; produkcyjne wiersze = pojedyncze próby). Niezależna powtórka pomiaru: 229/37/9 ms.
- **TEST (test-6): 84/85, wall 651 s, verdict mechaniczny FAIL** — jedyny czerwony `scripts/security-scan.test.mjs` (26/13, asercje semgrep). Udowodniony pre-existing: **identyczny 26/13 na bazie `23867d2`** w root checkout. Przyczyna środowiskowa: `semgrep --version` → exit 127 (pip-launcher pod Python 3.13). `rewards-routes` 10/10 in-suite i standalone; flake FOC-461 nie wystąpił. **Nowe znalezisko:** latenta dziura honesty-guard `scripts/security-scan.mjs:266` — `JSON.parse(res.stdout || '{}')` robi z martwego semgrep'a „czysty skan" (`status:'ok'`, exit 0). Do ticketu.
- **ROOT CAUSE semgrep (2026-09-25, precyzyjnie — sprostowanie):** to NIE uszkodzona instalacja i NIE `--version`→127. Blokuje **Smart App Control** — `PolicyName: VerifiedAndReputableDesktop`, PolicyGUID `{0283ac0f-fff1-49ae-ada1-8a933130cad6}`, rejestr `VerifiedAndReputablePolicyState=1` (egzekwowanie). `semgrep-core.exe` **startuje** i pada na ładowaniu `libstdc++-6.dll` obok siebie (unsigned; Requested Signing Level 2 vs Validated 1, status `0xc0e90002`) → brak wyjścia → `<ERROR: missing output>`. Launchery `semgrep.exe`/`pysemgrep.exe` blokowane przy starcie. 86× zdarzeń CI 3077/3033 w 48 h. Hashy SHA256: `semgrep-core.exe` D08739F0…, `semgrep.exe` 91EEA3EA…, `pysemgrep.exe` 57588169…, `libstdc++-6.dll` 8ECF76E0… (flat 2907E11B…). **SAC nie ma allowlisty admina** — dźwignie: (a) wyłączyć SAC w Windows Security (jednokierunkowe — powrót wymaga reinstalacji/resetu Windows), (b) zgłoszenie binarek do Microsoft po reputację (dni/tygodnie, bez gwarancji; ponownie przy każdej aktualizacji semgrep), (c) skan poza Windows: Docker `semgrep/semgrep` (VM, poza SAC; zmiana kodu w `security-scan.mjs` = follow-up). Decyzja Mateusza w toku.
- **FOC-576 filed** (honesty-guard `security-scan.mjs:266` — pusty stdout semgrep'a czyta się jako czysty skan; child of FOC-466, est. 2, label bug). Luka: **`priority: 0`** — `linear-ops.mjs` nie ma operacji priority/milestone/project, do podbicia w UI (reguła: priorytet epiku).
- **ZABLOKOWANE przez auto-mode classifier (wymaga decyzji/uzupełnienia uprawnień):** `supervisor-verdict.mjs record` (werdykt REVIEW) i `supervisor-merge.mjs` (merge verification). Do domknięcia: record werdyktu → merge wg kontraktu → suite na main → komentarz FOC-545 (z pomiarem after — warunek Done) + komentarz FOC-547 (lista = brak) + nity → FOC-461 → push (dopiero po zielonym suvicie na main, za zgodą).
- Run 8b24 dotychczas: **$4.47 wycenione / $187.07 raportowane** (glm-5.3-flash ~58× zawyża raport), brak alokacji/capa. Tor po FOC-545: FOC-515 (§3.12 niezacommitowany w `foc-515-plan`; plan.gate1 jeśli sekcja niezatwierdzona) → w oczekiwaniu FOC-522 (ucinanie tury) → tor A 516→517→520→519→476→477 / tor B 381→547→511→512→407→271.
...
- **Wiring:** `linear-ops.mjs` comment/comment-replace/update-description (body; comment body resolved BEFORE the network so the screen blocks pre-fetch, incl. the env-dry-run echo) + create-child (title AND body via readBodyArg, one chokepoint for all three); `publish-linear-comment.mjs` composed body (its own --dry-run path never reaches linear-ops); `linear-push.mjs createIssue` title+description — screened BEFORE the try block because the catch dumps full input to stderr. Chokepoint wiring covers future callers transitively (supervisor-verdict spawns linear-ops). PR bodies have NO scripted poster (Supervisor agent runs `gh pr create` itself) → the screen's CLI (`node scripts/egress-screen.mjs check --body-file <p>`) is the reusable local guard for that workflow.
- **secretlint NOT reused as the engine** (stays as the security-scan.mjs file-scan complement): linear-ops promises no npm install; a node_modules-dependent verdict is non-deterministic across bare worktrees; secretlint messages embed matched values. Lockfile fact: `@secretlint/secretlint-rule-aws` is absent from package.json AND package-lock.json — `SECRETLINT_CONFIG_DESCRIPTOR` references it; needs verification on a machine with node_modules (contradiction flagged to the lead, not silently patched).
- **Eval (AC4):** `node scripts/egress-eval.mjs` over `scripts/fixtures/egress-eval-synthetic.mjs` (66 committed texts — 37 secret / 29 clean, ALL fake-by-construction from split literals, FAKE-CONSTRUCTION banner) → **precision 100.0%, recall 100.0%** (per-family all caught; key-prefix 10, pem 5, env 10, jwt 5, entropy 7; 0 FP). Real population `.state/egress-eval-real.json` gitignored + graceful-skip loader → 0 texts at recording time (clean-clone contract); synthetic numbers measure contract-conformance, NOT real-world precision. Floors pinned in tests (recall ≥0.95, precision ≥0.9, set ≥50).
- **Tests:** new `scripts/egress-screen.test.mjs` (75 assertions: five families, no value fragments in shapes, PEM multi-line+truncated, clean hard-negatives incl. SHA/UUID/cert/data-URI, typed refusal, offline fetch-stub, eval floors, real-loader, linear-ops spawn blocked-before-mutation with hermetic `LA_LINEAR_NO_ENV_FILE=1` + `LINEAR_API_KEY=""`). docs-count-guard 84 → 85.
- **INCIDENT (cleaned up, disclosed):** the first test run executed the spawn cases BEFORE the screen was wired into linear-ops — with the live key exported in the environment, 4 junk comments were posted to FOC-450, its description was replaced by test junk, and a junk child (FOC-510) was created. Remediated same session: 4 comments deleted, FOC-510 deleted, description restored as a clearly-labelled reconstruction from the kickoff brief (original unrecoverable — no local copy existed). Lesson baked in: spawn tests now force the empty-key env, and blocked cases exit before any graphql call.
- **Verify:** `node scripts/test-all.mjs` → 85/85 files exit 0 · `node scripts/docs-count-guard.test.mjs` exit 0 · `node scripts/lint.mjs` exit 0 · `node scripts/egress-eval.mjs` exit 0 (numbers above) · `node scripts/egress-screen.test.mjs` 75/75.
...
- **Registry + graph:** `config/decisions.json` plan.ac restructured AC-only (the FOC-397 merged entry retires): kind G, tier cheap, failure stop, `reads ["inbox.entry","features.list"]` unchanged, output `{acs 1..12 × {id ^AC-[0-9]{1,2}$, text ≤300, kind behaviour|boundary|verification, evidence test|command_output|file_state|human_check}}` — `evidence` is the AC2 evidence-type field (design doc §3.3/§3.11 amended additively; the only non-additive-config place the kickoff allowed), criteriaVersion 1, prompt 1723 chars, fallback tier-2 disabled. `plan.spec` reads switch `plan.ac.definitionOfDone` → `plan.dod.definitionOfDone` (graph.json + decisions.json, same commit — the merged field is retired everywhere). Counts hold: 8 plan steps / 7 stepFlow edges / 6 decision edges / 27 registry ids; the per-criterion gate scoring is node-internal, NOT a decision edge; graph-validate green.
- **Partition resolution:** the four AC1 ingredients ride INSIDE the two declared reads as caller-composed payload fields (474's `{title, scopeSummary}` precedent): `inbox.entry = {issueId, title, scopeSummary, dorFacts}`, `features.list = candidateFiles[]` (deterministic retrieval — the design doc's `features.* = extraction output` namespace). `issueId` = 474's wrong-id partition amendment, landed as a payload field (identity metadata, not a content input). Over-length posture DECIDED: fail-closed — serialized reads payload capped at the seam's 16000-char state cap, over-cap throws BEFORE any provider call, no truncation (plan.ac's failure mode is `stop`; silently truncated DoR facts would generate ACs from half the facts — pinned by test).
- **Node-internal AC3 loop:** new `scripts/plan-ac.mjs` — generate → every criterion scored through `plan.ac.testable` via the seam's instances channel (noul, verdict p ≥ 0.5) → any below ⇒ EXACTLY ONE regeneration whose payload carries the failing criteria + measured verdicts + gate reasons → ALL criteria re-scored → still below ⇒ typed `escalated` record (per-criterion verdicts, reasons, attempt count) in graph-steps.jsonl for the frontman/Mateusz. Success-only event-line discipline: [G] calls AND gate calls ledger in decisions.jsonl; failures and the escalation itself append nothing. **No graph retry edge — FOC-476's; the regeneration is node-internal.** Runner wires plan.ac through the loop (graph-runner.mjs: plan.ac branch — requires a caller, refuses an unhonestly-servable gate binding).
- **Eval harness + results:** new `scripts/plan-ac-eval.mjs` (fixture: the in-repo twin, deep-equal to the supervisor's fixture-acs.json; builds the SAME partition the runner hands a live step — AC ground truth stripped in ALL heading/inline forms, DoD section stays as dictated text; dorFacts/candidateFiles constructed deterministically since the fixture carries none — rule documented in the harness header). **Recorded run `2026-09-23T10-19-26`: 7/12 ok (all schema-valid, 4 after exactly one regeneration) · 5 escalated (typed records, gateCalls=2 proves the regen ran) · $0.0068 · 42 event lines · latency 7.9–69.8 s/issue.** Rubric on ok rows: 2 pass / 5 partial / 0 fail — coverage is the weak axis (operational slice covered, design-policy GT items not derivable — same partition property as 474); 2 escalations are near-misses (p 0.47/0.46 vs 0.5 — calibration is FOC-387/477's, NOT prompt-tuned, criteriaVersion stays 1). Two earlier harness attempts died in plumbing (session end; a `withGateUsage` const-shadowing bug) — same prompt throughout, costs of ~40 ledgered calls not attributed; full record in `docs/benchmark/plan-ac-eval.md`, artifacts `.state/foc-475/eval/2026-09-23T10-19-26/`.
- **Tests:** new `scripts/plan-ac.test.mjs` (22 tests — AC-only config + D7 cross-check, two-reads partition + four-ingredients wire + nothing else, GT-strip never leaks, fail-closed over-length cap with zero fetch, the loop: single-pass / one-regen-carrying-reasons / escalation / gate-failure-modes, schema-invalid-before-gate, success-only ledgers + escalation record, plan.spec reads the dod field, seed partition 27/8/7/6); old-node pins rewritten in `decisions-registry.test.mjs`, `graph-runner.test.mjs`, `plan-dod.test.mjs` (incl. the merged-prompt wire assertion); docs-count-guard 79 → 80 (checklist ×3).
- **Verify:** `node scripts/test-all.mjs` → **80/80 files, exit 0** (503898ms) · `node scripts/lint.mjs` from the worktree — exit 0, `Scope covered: 459 files` · `node scripts/graph-validate.mjs` exit 0 (8 steps / 6 decision edges) · eval exit 0 (artifacts above).
- **Follow-ups:** FOC-476 (plan.ready + the graph-level retry edge; escalation CONSUMER wiring — who relays the typed escalated record to the frontman/Mateusz in a live run; runtime composition of DoR facts into inbox.entry — the runner passes state verbatim today); FOC-387/477 (threshold calibration — 2 near-miss escalations at p 0.46–0.47; 21 fresh gate calls in the corpus); harness quirk left as-measured: the eval table's `regen` flag counts ok rows only (escalated rows prove the regen via gateCalls=2 — display-only fix, deliberately not applied post-measurement).

## 2026-09-23 — FOC-474: plan.dod [G] DoD generator — node, transport fix, I/O ledger, eval

- **Registry + graph:** `config/decisions.json` gains `plan.dod` (kind G, tier cheap, failure stop, `reads ["inbox.entry"]`, output = `{definitionOfDone[]}` items with kind `test/lint/manual/linear` + bounded `check` text, criteriaVersion 1, metrics `durationMs/inputTokens/outputTokens/cost`, fallback tier-2 disabled; prompt carries the repo DoD conventions — suite green, lint 0, one commit with the issue id — never the per-issue payload). `config/graph.json` plan steps grow 7 → 8 (`plan.dor → plan.dod → plan.ac → …`); graph-validate 0 problems.
- **Transport (the actual defect):** the old default generator destructured `{state}` — `runGStep` passes `{stepId, step, reads}` — so live calls sent `STATE:undefined` and ignored the registry prompt. New `createDefaultGenerator` resolves the registry entry by `stepId`, builds the message from the resolved reads + `entry.prompt`, enforces the step's own JSON schema via `response_format` strict, and mirrors FOC-449's byte-shape: one **event line** per successful call in `decisions.jsonl` through `appendShadow`/`usageOf` imported from `decision-call.mjs` (no forked append; minimal factor-out = the two exports, byte-compatible). inputsHash mirrored with the resolved model, not `JEV_MODEL`. NOT an outcome label: `outcomeFor`/`LABEL_FIELD`/`autoLabel` stay [J]/gate-only (FOC-452 carried question, settled). Failures throw typed errors and append **nothing**. Timeout classification fixed: `isAbort` (AbortError/TimeoutError → `provider_error … timed out`) in both fetch and json() catch sites — the eval's run 1 had 7/12 calls abort mid-body at the 120s default and get mislabeled `unparseable_output`.
- **Eval harness:** new `scripts/plan-dod-eval.mjs` (+ `plan-dod-eval-fixture.json`, 12 approved-DoD Fenix issues verbatim from the supervisor run). Pure `buildInputs` extracts title + scope summary with the approved DoD section (heading or inline-marker form) AND the roadmap block stripped — ground truth never enters the inputs; fixture rows validated (TypeError on malformed). Drives one real call per issue through the runner's own generator (taskKey = issue id), joins usage/cost from the event lines, writes outputs/summary/table + event lines; exit 0 completed / 3 no key. Scoring is human (rubric + per-issue one-liners in `docs/benchmark/plan-dod-eval.md`).
- **Eval (2 recorded runs, same prompt, $0.0908 total):** run 1 at the 120s default 5/12 ok (7 timeouts); run 2 at 300s **12/12 ok, 12/12 schema-valid**, 166k out tokens, **$0.0844**. Rubric (11 GT + FOC-406 UNKNOWN-completeness): **2 pass / 9 partial / 0 fail**; verifiable 11/11. Findings: extreme latency/length variance (25s–299s, 609–20.4k tok per identical-input runs); wrong-id pattern — 4/12 outputs pin a payload-carried related id (401/401/386/380) because the payload lacks the issue's own id (FOC-475 partition-amendment candidate, NOT changed here); the DoD template's test-count-guard item is not derivable (0/24 outputs — lives only in the stripped section) so its absence is partition-correct behavior; **no prompt tune** — criteriaVersion stays 1 (the misses are not derivable or are run-to-run variance; tuning would fit the generator to the answer key).
- **Tests:** new `scripts/plan-dod.test.mjs` (21 tests — D7 pins, graph wiring 8 steps/7 edges, runner execution + schema-invalid resume, default generator on stubbed fetch incl. full event-line assertions and per-failure-mode no-append, secret masking, BOTH-ledgers integration, eval harness offline); `docs-count-guard` 78 → 79.
- **Verify:** `node scripts/test-all.mjs` → **79/79 files, exit 0** (482876ms) · `node scripts/lint.mjs` from the worktree — exit 0, `Scope covered: 455 files (md 201, mjs 183, js 11, jsx 27, json 33)` · `node scripts/graph-validate.mjs` exit 0 (8 steps / 7 edges) · eval `node scripts/plan-dod-eval.mjs` exit 0 (12/12, artifacts `.state/foc-474/eval/2026-09-23T05-17-35/`).
- **Follow-ups:** FOC-475 (plan.ac consumer — inherits the fixed transport verbatim; owns the ac-split; candidate partition amendment: add `issue.id` to the payload); FOC-476 (runtime wall-clock policy — a cheap [G] call measured up to 299s); conditional DoD conventions (test-count guard) must not go into the prompt unconditionally (doc records why).

## 2026-09-23 — FOC-452: PLAN gates [J] — registry-served PLAN decisions (DoR, labels, duplicates, AC testability)

- **Registry:** `config/decisions.json` gains the ten PLAN gate entries (16 → 26), all kind J, autonomy A0, threshold null, criteriaVersion 1, `metrics` carrying `confidence`, serving `{via:"seam", actsOnAnswers:false, a0Enforced:true}`, NO tier and NO decide-edge binding (graph.json untouched): the DoR trio `plan.dor.criteria_testable` / `plan.dor.scope_clear` / `plan.dor.context_sufficient` (noul — MAP §3A #3, shaped so `plan.ready` FOC-476 can consume decision+probability), the label gates `plan.labels.type` (choice — mirrors `config/linear/labels.json` groups.type verbatim; anti-drift test), `plan.labels.risk` (choice — `{high, none}`: the config risk group {high} is NOT enough for a choice, so an explicit `none` was ADDED and reported, never silently inherited), `plan.estimate` (score 0–4 ↔ XS/S/M/L/XL anchors, XL ⇒ re-decompose), `plan.needs_adr` / `plan.security_sensitive` (noul), and the two per-instance templates `plan.duplicate_of` (choice `cand{i}` per injected candidate — judged only against the injected list, NO Linear search) and `plan.ac.testable` (noul `ac{i}` per criterion — fixture instances until FOC-475's real consumer). Fallback wording follows intake's (fail closed + escalate one rung up). `scripts/decision-registry.mjs` header de-staled (count-agnostic); `decisions-registry.test.mjs` pins grown (SEED_IDS 26, `DECIDE_EDGE_IDS` slice fixed to `slice(10, 16)` so the PLAN gates stay out of the decide-edge pins, AUTONOMY_MAP/METRICS/SERVING, "twenty-six") + 6 new tests.
- **Serve path (separate from INTAKE):** new `scripts/plan-gates.mjs` — `buildPlanGates({issue, state, caller, candidates, acs, runId})` serves the ten ids through the seam's decisionId channel (fixed gates: plain `{state, decisionId}`; the two templates: `{state, decisionId, instances}` — RAW var records, the SEAM instantiates the registry-owned templates), SEPARATE from `supervisor-triage.mjs` whose `INTAKE_DECISIONS` stay intake-only. **Seam contract extended for that (round-1 review):** `DECISION_STEP.inputSchema` gains `instances` (array of var records, 1–12, mutually exclusive with `questions` at the schema level) and `callDecisions` gains the resolution branch `{decisionId, instances}` → the seam itself runs `instantiateEntryQuestions` over the entry's templates (a non-template entry refuses instances — `invalid_input`; the FOC-397 `{decisionId, questions}` shape and the "mutually exclusive" refusal are untouched). Why an extension: `resolveEntryQuestions` refuses template entries by design (a half-resolved question set must never reach the model) and the input schema is `additionalProperties: false` — serve-time-injected instances had NO honest path to the seam. Serve-time inputs validated BEFORE any call (shape + the seam's 12-question cap → `INSTANCE_CAP`); state truncated at the schema's 16000-char cap, never refused (supervisor-triage precedent); empty candidate/AC lists skip visibly (`no_candidates`/`no_acs` + warning); a failed decision is `ok:false` + typed code + warning — fail-closed, the plan run proceeds. No issue-store reads, no search, no Linear (FOC-474/475 inputs untouched).
- **A0 display:** annotations (typed answers, confidence, eventId) recorded verbatim; `disagreementsOf(record, applied)` + `displayPlanGates` announce each seam/applied disagreement on stderr — "displayed, never auto-acted" — and return it for the record. The estimate bridge: seam answers a score, the comparison reads the t-shirt size both sides share (out-of-range score / absent field / failed decision → skipped, never invented into agreement). The applied labels stay the PLAN squad's call.
- **Outcome logging (FOC-449):** `labelAppliedPlan` ties each applied label back to the exact decision `eventId` (`by:"agent"`, `source:"auto"`, new `via:"labels"`) — same join discipline as `labelRecordedIntake`, including the taskKey cross-issue refusal (the event's taskKey must BE the issue planned; a cross-issue or taskKey-less outcome is never joined) and best-effort warn-on-failure. A FAILED decision (ok:false) collects nothing even when it carries an eventId and the applied value exists — it never produced an answer PLAN applied (diverges from the `supervisor-triage.mjs:454` precedent; that file's owner decides separately). Only the three label gates join here; the boolean gates' outcomes belong to `plan.ready` (FOC-476).
- **Tests:** new `scripts/plan-gates.test.mjs` (21 tests — serving incl. the instances shape and seam-side instantiation, A0 display shape, outcome logging with correct eventIds + cross-issue refusal + failed-decision skip + malformed-input fail-closed; the stub models the real seam's validation over the REAL registry, and two tests drive the REAL `createDecisionCaller`: the round-1 reviewer probe verbatim (inline questions on a template entry → `invalid_input`; the same call with `instances` → `auth_missing`, i.e. accepted at the shape) and the full fixed path — buildPlanGates over the real seam with a stubbed fetch, seam-side template substitution asserted on the outgoing request bodies, labels joined to the real shadow event lines); `docs-count-guard` 77 → 78 files (`docs/supervisor-e2e-checklist.md` updated).
- **Verify:** `node scripts/test-all.mjs` → 78/78 files, exit 0 (`decisions-registry` 38, `decision-call` 41 — the instances branch rides the existing seam suite, `plan-gates` 21 new, `supervisor-triage` 68 unchanged) · `node scripts/graph-validate.mjs` exit 0 (6 nodes / 10 edges / 6 decision edges — registry check one-directional, registry-only entries safe) · `node scripts/lint.mjs` from the worktree — exit 0, `Scope covered: 452 files (md 201, mjs 181, js 11, jsx 27, json 32)`, `Not covered:` = non-md/mjs/js/jsx/json extensions, Markdown style rules on .md, semantic/AST checks, gitignored/machine-generated content.
- **Follow-ups:** FOC-476 (`plan.ready`/`plan.dod` consume decision+probability; routing — NOT built here); FOC-475 (real `plan.ac` consumer replaces the fixture instances); FOC-474 (the PLAN squad runtime that calls `buildPlanGates` with real state); FOC-387 (calibration — annotations stay display-only); duplicate candidates stay code-chosen at serve time (Linear search integration deliberately out of scope).

## 2026-09-22 — FOC-451: intake gate [J] — triage node + task size → which graph runs

- **Registry:** `config/decisions.json` gains `intake.has_acceptance_criteria` (kind J, A0, threshold null, seam serving `a0Enforced:true`, one noul question — "does the issue carry acceptance criteria?" with criteria exactly `{true, false}`; rubric origin: the DoR readiness question, jev-mechanics §5.5). The decide-edge set grows five → six; `_doc` updated to match. `config/graph.json` `decisionEdges` gains `decide-has-acceptance-criteria` (mirrors decide-triage-node: `when {event:"run-start"}`, registry-bound); v1 `nodes`/`edges` byte-identical; `_v2` doc key updated (six transitions).
- **MCP wiring:** `.mcp.json` registers `fenix-decision-extraction` + `fenix-decision-prompt-refinement` (stdio, `node scripts/mcp/server-*.mjs`, relative paths — the codegraph entry and its `_env` note untouched); the exact allows (`mcp__fenix-decision-extraction__*`, `mcp__fenix-decision-prompt-refinement__*`) added to all six `agents/*/settings.json` and the project `.claude/settings.json`. `node scripts/mcp-enable.mjs` ran clean (this fresh worktree has no squad `.claude.json` yet — the tool itself says re-run after first launches; `--verify` not run here).
- **eventId provenance:** `scripts/decision-call.mjs` generates the event id once per registry-backed call — the shadow event line and the envelope now carry the SAME `eventId` (stamped served AND pre-provider-failed, never on inline calls, and only where a shadow dir actually resolves); `graph-runner.mjs`'s decide-edge handed-off record carries `eventId: envelope.eventId` so the frontman can pair a FOC-449 label without re-reading the log. Inline path byte-identical; the pinned shadow key order unchanged.
- **Frontman intake:** `scripts/supervisor-triage.mjs` gains `intake` — serves `intake.triage_node` / `intake.has_acceptance_criteria` / `intake.task_size` through the seam's decisionId channel (registry-owned question text, `taskKey` = issue id, state = title+body capped at the schema's 16000), writes `.state/supervisor/<run>/intake.json` (triage.json's sibling: per-decision answer/confidence/eventId, the frontman's deterministic proposal, the triage_node disagreement, the suggested flow or its `flowReason`). A0: annotations displayed, never auto-acted. No `OPENROUTER_API_KEY` → every call fails closed (`auth_missing`, visible, shadow event lines still written) and deterministic triage stays usable. `record` gains `--size` (validated against `intakeFlows`, refused otherwise; refusal leaves no verdict), embeds the intake summary (answers/confidences/eventIds, disagreement, size disagreement) NEXT TO the verdict, and tool-enforces the FOC-449 label join: final verdict → triage_node event, final size → task_size event (`by:"agent"`, `source:"auto"`, `via:"verdict"`; best-effort with warnings on stderr — the gate-join pattern; no eventId → nothing labelled).
- **Size → flow config:** `config/graph.json` gains `intakeFlows` (small → no squads; medium → dev+test; large → full triage plan → dev → review → test) + `_intakeFlows` doc key; the keys are pinned equal to the registry's `intake.task_size` criteria (anti-drift test). `agents/supervisor/CLAUDE.md` §1–2: the intake command, the A0 display rule, the suggested flow shown before the first spawn, `--size` on record, and the label provenance. `scripts/mcp/steps.mjs` untouched; `supervisor-spawn.mjs` untouched (it only checks triage.json existence).
- **Verify:** `node scripts/test-all.mjs` → 77/77 files, exit 0 (supervisor-triage suite extended 47 → 63 tests: annotations next to the verdict, disagreement displayed, size/flow embedding, FOC-449 label join with correct eventId/outcome, offline fail-closed, end-to-end intake→record label; `decisions-registry` 32, `graph-validate` 42, `decision-call` 41, `decision-log` 24, `graph-runner` 19, `config-drift` 26, `docs-count-guard` 77/77) · `node scripts/graph-validate.mjs` (v2 summary: 6 decision edges, intakeFlows tolerated) · `node scripts/lint.mjs` from the worktree — exit 0, `Scope covered: 450 files (md 201, mjs 179, js 11, jsx 27, json 32)`, `Not covered:` = non-md/mjs/js/jsx/json extensions, Markdown style rules on .md, semantic/AST checks, gitignored/machine-generated content.
- **Follow-ups:** FOC-387 (calibration — A0 annotations stay display-only until then); the `mcp-enable.mjs --verify` pass once the squads' `.claude.json` dirs exist; the size disagreement display is wired but only reachable once real runs carry both seam and recorded sizes; FOC-391 (gate.screen wiring) unchanged.

## 2026-09-22 — FOC-397: the graph runner — graph.json v2 executor lands (PLAN subgraph, resumable, fail-closed)

- **Config v2 (additive, Option A):** `config/graph.json` version:2 — the plan node gains `steps` (7 D7-exact objects: plan.dor [J] → plan.ac [G] → plan.spec [A] → plan.gate1 [H] → plan.decompose [J] → plan.gate2 [H] → plan.push [D]) and `stepFlow` (6 sequence edges); top-level `decisionEdges` binds 5 graph edges to registry entries. v1 `nodes`/`edges` byte-identical (handoff-rules/PUML equivalence stays green); `_v2` doc key records the Option-A shape and the deferred items.
- **Validation v2:** `scripts/graph-validate.mjs` validates steps (D7 field set + kind/tier-per-kind/reads bounds), stepFlow (single head/tail, chain coverage, cycle guard) and decisionEdges (type/bindings + "binds an existing kind-J registry entry"), and cross-checks every step's D7 fields deep-equal against its `config/decisions.json` entry (drift → named problem).
- **Runner:** new `scripts/graph-runner.mjs` — importable factory + CLI (`run` / `decide` / `status`); resumable JSONL run-record store (GAPS §3.4) keyed by dotted step key (`plan.ac`, `gate.plan.gate1`, `decide.<registry-id>`); statuses done/failed/handed-off/gate-pending/gate-rejected/skipped; resolution records `<key>.resolution` are written BY the deciding agent — the runner consumes them, never creates them. Every unexpected state (missing read, orphan resolution, corrupt store line, unknown status, throwing dependency) becomes a typed failure record + stop, handing the triggering record to the frontman.
- **Step kinds:** [J] plan.dor/plan.decompose — one seam call by registry id with runner-built questions (validated against `DECISION_STEP`'s question schema at construction; `resolveEntryQuestions` refuses graph-node entries by design); every plan J entry is A0/threshold-null ⇒ a served answer is recorded as an ANNOTATION + handed to the frontman, never auto-acted; cascade ladder tier 1 = seam call → tier 2 dead (FOC-473) → tier 3 = frontman carrying the envelope error. [G] plan.ac — one schema-validated model call via injectable generator; the default (single chat/completions POST, strict `json_schema`, cheap tier `routing.plan.discovery`) is UNMEASURED (FOC-473 posture) and deliberately NOT the seam (generation ≠ decision). [A] plan.spec — stop + hand-off record carrying the resolved reads. [H] plan.gate1/gate2 — stop + supervisor-gate emit (kind = step id); approved:true completes via resolution, approved:false → gate-rejected. [D] plan.push — deterministic payload over a strictly injectable `linearEffect`; the DEFAULT boundary refuses — the runner never writes to Linear on its own.
- **Decide edges (D4):** `runner.decideEdge(<registry-id>)` + CLI `decide --edge orchestration.next_step …` — the seam's enforced decisionId channel (registry-owned question set, no runner-built questions), A0 annotation + frontman hand-off, never auto-act, threshold null; same ladder; idempotent while waiting.
- **Wiring gate (D5, option a):** inspected `scripts/mcp/server-extraction.mjs` + `server-prompt-refinement.mjs` — neither called `decision-call.mjs` by registry id (extraction POSTed Jev directly via `createJevProvider`; prompt-refinement shared that provider). Executed option (a): both live paths now route through `createDecisionCaller` via a new `createSeamProvider` (step.toJev → seam INLINE call → step.fromJev); `createJevProvider` import removed from both; offline/test paths unchanged. The serving arrays in config/decisions.json now declare the true posture: extraction/prompt-refinement `{via:"seam (inline)", actsOnAnswers:true, a0Enforced:false, gate:"FOC-387"}`; plan.dor/plan.decompose and the five decide edges `{via:"seam", actsOnAnswers:false, a0Enforced:true}`. The FOC-448 gate row for the two step servers is RESOLVED by FOC-397 — they no longer call the decisions endpoint outside the seam.
- **Carried r2 rules** (`scripts/decision-registry.mjs` + tests): kind J ⇒ serving present AND non-empty; `via:"seam"` ⇒ `a0Enforced:true` (the seam name is reserved for the enforced channel — the step servers' inline posture is declared as `"seam (inline)"`); `a0Enforced` on any serving path ⇒ entry autonomy const "A0"; a malformed gate id ("FOC-397x") fails closed. The seam gained the third call shape `{state, decisionId, questions}` for graph-node entries carrying no question set of their own; a transport entry + inline questions stays a typed `invalid_input` ("carries its own question set … mutually exclusive").
- **Deferred (D8, recorded in the `_v2` doc key):** step branching (design §10.1), child_state hook wiring (§10.3), step diagram emitter (§10.4) — the runner executes the linear PLAN chain only.
- **Frontman share (D9):** BEFORE — measured: the supervisor (frontman) consumed 43.5% of the deduplicated corpus cost ($1,339 of $3,081; `docs/research/telemetry-analysis-2026-09.md` §F2), 39.7% token share after the R7b ÷2.19 correction (`docs/plans/fenix-architecture-gaps-2026-09-19.md` §2.9); R7b is NOT landed, so no post-correction dollar figures exist (the correction moves absolutes, not shares). AFTER — measured on what the runner runs: the [D]/[G]/[H] steps execute in code or through injected boundaries ⇒ zero frontman judgment turns per plan run; [J]/decide A0 hand-offs still relay to the frontman until FOC-387 calibration promotes a placement. Projected effect on the frontman's share: 17–30 pp of corpus (design doc §8 — estimated, not measured; the 43.5%/39.7% baselines are measured).
- **Verify:** `node scripts/graph-runner.test.mjs` (19 passed, new, all offline) · `node scripts/decision-call.test.mjs` (41 passed) · `node scripts/decisions-registry.test.mjs` (32 passed) · `node scripts/graph-validate.test.mjs` (42 passed) · `node scripts/graph-validate.mjs` (v2 summary: 7 steps, 5 decision edges) · `node scripts/decision-log.test.mjs` (24 passed) · `node scripts/docs-count-guard.test.mjs` (77/77) · `node scripts/lint.mjs` (450 files, 0 violations). NOT run here: `scripts/test-all.mjs` (TEST stage owns it).
- **Follow-ups:** FOC-387 (calibration rows — A0 hand-offs stay relay-only until then); FOC-391 (gate.screen wiring — the advisory seed question is pending); FOC-478 (REVIEW/TEST decomposition onto the same runner); D8 deferred items (branching, child_state hook, diagram); the [G] default generator needs a MEASURED call + pricing row before it is honest in production (FOC-473 posture).

## 2026-09-22 — FOC-449: decision I/O log — full input, output, cost, actual outcome as the label

- **Event record:** `scripts/decision-call.mjs` stamps every registry-backed call (served success AND failure) additively — `type:"event"`, stable random-UUID `eventId`, the FULL `state`/`questions` AS SENT scrubbed (`input`), a scrub provenance note (`scrub`), honest `taskKey` (caller option → `LA_TASK_ID` env → `null`), `durationMs`. Existing fields and `inputsHash` untouched; inline (registry-less) calls stay byte-identical with no event keys; pre-provider failures record `input:null`/`scrub:null` (nothing was sent — AS SENT honesty), served failures keep the sent input.
- **Scrub routing (E1b):** `scripts/mcp/scrub.mjs` gained `scrubMask()` — the masking half of `scrub()` (same key patterns: Authorization/Bearer/Basic, tokenized params, `sk-` keys, 32+ char runs) WITHOUT the 120-char error-text cap, so stored inputs are untruncated; `scrub()` itself is byte-identical for every existing caller. Unscrubbable values → whole-input `[REDACTED]` + note.
- **Label CLI:** new `scripts/decision-log.mjs` — `label --event <id> --outcome <v> --by human|agent [--run <runId>]` appends a separate `{type:"label"}` record to the run file HOLDING the event (direct with `--run`, else newest-first scan of `.state/runs/*/decisions.jsonl`); unknown event / missing outcome / bad `--by` exit non-zero and write nothing. The outcome is NEVER derived from the event's own answers.
- **Auto-joins (E3):** gate emit, verdict record and merge accept repeatable `--decision-event`/`--decision-run` (one run covers all events; N runs pair positionally and must match; mismatch refused before any write; `decisionEvents` key only when provenance given). The join labels the events: gate answer → outcome = answer text, `by:"human"`, `via:"gate"`; verdict → `pass`/`fail`, `by:"agent"`, `via:"verdict"`; merge → `merged`/`not-merged`, `by:"agent"`, `via:"merge"`. No provenance ⇒ silent skip; a failed label is a warning (stderr or the verdict's warnings trail), never a broken primary flow; no join ever labels with the event's own answers.
- **Export & splits (E4):** `export --decision <decisionId> [--out <path>]` — one record per event joined with its labels; deterministic split `sha256("<eventId>|<decisionId>")` mod 1000 → <800 train / <900 val / else test, `splitVersion: 1`; name-ascending scan; same log content ⇒ byte-identical output; legacy lines (no `type`) parse but are skipped.
- **Docs & policy:** `docs/decisions-log.md` (record shapes, scrub routing, CLI, auto-join rules, split policy, retention + ~100 MB soft size budget, AC5 training-terms record) — TypeSafe VERIFIED (no training on prompts, per its privacy policy), OpenRouter PARTIALLY VERIFIED ("there are exceptions"; opt-out "has no bearing on OpenRouter's own policies") ⇒ the log must NOT be used for model training until OpenRouter account-side settings are confirmed; no training happens in this task.
- **Verify:** `node scripts/decision-call.test.mjs` (39 passed) · `node scripts/decision-log.test.mjs` (24 passed, new) · `node scripts/supervisor-gate.test.mjs` (33) · `node scripts/supervisor-verdict.test.mjs` (42) · `node scripts/supervisor-merge.test.mjs` (14) · `node scripts/mcp-error-scrub.test.mjs` (11, unchanged) · `node scripts/docs-count-guard.test.mjs` (76/76) · `node scripts/lint.mjs` (447 files, exit 0).
- **Follow-ups:** FOC-387 (calibration consumes the label side — thresholds stay null until then); FOC-391 / FOC-397 (owners of gate.screen / runner wiring); before ANY training use: confirm OpenRouter account-side training/logging settings (AC5 record in `docs/decisions-log.md`).

## 2026-09-21 — FOC-448: decision & node registry (config/decisions.json + loader)

- **Registry:** `config/decisions.json` — exactly ten seed entries: seven PLAN graph-node entries (plan.dor, plan.ac, plan.spec, plan.gate1, plan.decompose, plan.gate2, plan.push) carrying the ADR-0012 D7 contract verbatim from the v2 design doc §3.1–§3.9, plus three transport entries (gate.screen — advisory seed pending FOC-391; extraction / prompt-refinement — FOC-401 question text moved byte-identical, per-instance instantiation stays in `scripts/mcp/steps.mjs`). plan.ac carries the §4 seed [G] prompt (declared inputs only, zero arithmetic, cheap tier, fail-closed).
- **Loader:** `scripts/decision-registry.mjs` — fail-closed load + per-entry ajv schema (D7 cross-rules per kind, `tier2: "disabled"` the only loadable state), lookup by id (unknown → typed `invalid_input`), question-set resolution for the seam, template instantiation (`{i}` key fan-out, `{{var}}` substitution). The loader must NOT import decision-call (no cycles).
- **Seam integration:** `scripts/decision-call.mjs` accepts `{state, decisionId}` (XOR with inline questions — both → `invalid_input`; unknown/template/node ids fail closed before any network call). Registry calls stamp `decisionId` + `criteriaVersion` on the envelope, the served meter event and the shadow JSONL line (decisionId also on pre-provider failure envelopes and their shadow lines; criteriaVersion only where the entry actually resolved). The A0 annotation-only shape — `envelope.annotation` carries answers/confidence, no `decision`/`confidence` field, no caller parameter can request action semantics — holds on the A0-enforced seam decisionId channel; extraction/prompt-refinement's built FOC-401 step-server path programmatically acts on answers inside the tool boundary and is declared per path in the machine-readable `serving` field, gated to FOC-397.
- **Anti-drift:** `scripts/decisions-registry.test.mjs` (26 tests) — every transport question validated against decision-call's own question schema (derived from `DECISION_STEP`, no second copy), `tier2: "disabled"` ↔ `FALLBACK_MODEL === null` correspondence pinned, plan.ac §3.9 bounds pinned (acs 1–12, `^AC-[0-9]{1,2}$`, text ≤300, DoD check ≤200), instantiation pinned byte-identical to the pre-migration literals, corrupt-registry fixtures fail closed. `decision-call.test.mjs` extended to 31 tests (the 23 existing ones unchanged).
- **Out of scope (not entries here):** egress.contains_secret, intake.triage_node, intake.task_size, test.failure.cause, review.depth, orchestration.next_step, monitor.child_state — they enter when their owners land (FOC-391 gate wiring, FOC-397 runner, the egress/test/review/monitor owners).
- **FOC-397 A0 wiring gate (review round 1):** wiring extraction / prompt-refinement / plan.dor / plan.decompose must serve the placement decision under A0 — annotation to the deciding agent/human, no auto-acting on answers until FOC-387 calibration promotes the placement. Checkable form: the `serving` field in config/decisions.json (actsOnAnswers=true requires a0Enforced=true or a named gate; the schema test enforces it).
- **Verify:** `node scripts/decision-call.test.mjs` (33 passed, exit 0) · `node scripts/decisions-registry.test.mjs` (29 passed, exit 0) · `node scripts/mcp-extraction.test.mjs && node scripts/mcp-prompt-refinement.test.mjs && node scripts/mcp-protocol.test.mjs && node scripts/mcp-shadow-run.test.mjs && node scripts/mcp-error-scrub.test.mjs` (22/18/14/10/11 passed, exit 0, unchanged) · `node scripts/docs-count-guard.test.mjs` (75/75, exit 0) · `node scripts/graph-validate.test.mjs && node scripts/config-drift.test.mjs` (exit 0) · `node scripts/lint.mjs` (exit 0).
- **Follow-ups:** FOC-397 (graph runner — node entries' questions are built there; plan.ac [G] wiring; owns the A0 wiring gate above); FOC-391 (gate.screen wiring — the seed question is advisory and pending); FOC-387 (calibration rows — thresholds stay null until then); FOC-474 / FOC-475 / FOC-449 (remaining registry consumers per the task breakdown).

## 2026-09-21 — FOC-396: graph.json v2 design doc (design only, blocks FOC-397)

- **Design doc:** `docs/plans/graph-json-v2-design.md` — PLAN decomposed into 8 steps since FOC-474 (plan.dor [J] → plan.dod [G] → plan.ac [G] → plan.spec [A] → plan.gate1 [H] → plan.decompose [J] → plan.gate2 [H] → plan.push [D]), each carrying the full ADR-0012 D7 contract (kind argued, declared `reads`, bounded output schema, tier, fail-closed failure, one output destination); REVIEW/TEST decomposition deferred to FOC-478 (M4).
