...
- **Serve path (separate from INTAKE):** new `scripts/plan-gates.mjs` — `buildPlanGates({issue, state, caller, candidates, acs, runId})` serves the ten ids through the seam's decisionId channel (fixed gates: plain `{state, decisionId}`; templates: `instantiateEntryQuestions` → third shape `{state, decisionId, questions}`), SEPARATE from `supervisor-triage.mjs` whose `INTAKE_DECISIONS` stay intake-only. Serve-time inputs validated BEFORE any call (shape + the seam's 12-question cap → `INSTANCE_CAP`); empty candidate/AC lists skip visibly (`no_candidates`/`no_acs` + warning); a failed decision is `ok:false` + typed code + warning — fail-closed, the plan run proceeds. No issue-store reads, no search, no Linear (FOC-474/475 inputs untouched).
...
- **Outcome logging (FOC-449):** `labelAppliedPlan` ties each applied label back to the exact decision `eventId` (`by:"agent"`, `source:"auto"`, new `via:"labels"`) — same join discipline as `labelRecordedIntake`, including the taskKey cross-issue refusal (the event's taskKey must BE the issue planned; a cross-issue or taskKey-less outcome is never joined) and best-effort warn-on-failure. Only the three label gates join here; the boolean gates' outcomes belong to `plan.ready` (FOC-476).
- **Tests:** new `scripts/plan-gates.test.mjs` (16 tests, all offline/stubbed — serving incl. template instantiation, A0 display shape, outcome logging with correct eventIds + cross-issue refusal + malformed-input fail-closed); `docs-count-guard` 77 → 78 files (`docs/supervisor-e2e-checklist.md` updated).
- **Verify:** `node scripts/test-all.mjs` → 78/78 files, exit 0 (`decisions-registry` 38, `plan-gates` 16 new, `supervisor-triage` 68 unchanged) · `node scripts/graph-validate.mjs` exit 0 (6 nodes / 10 edges / 6 decision edges — registry check one-directional, registry-only entries safe) · `node scripts/lint.mjs` from the worktree — exit 0, `Scope covered: 452 files (md 201, mjs 181, js 11, jsx 27, json 32)`, `Not covered:` = non-md/mjs/js/jsx/json extensions, Markdown style rules on .md, semantic/AST checks, gitignored/machine-generated content.
