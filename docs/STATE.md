...
- **Seam integration:** `scripts/decision-call.mjs` accepts `{state, decisionId}` (XOR with inline questions — both → `invalid_input`; unknown/template/node ids fail closed before any network call). Registry calls stamp `decisionId` + `criteriaVersion` on the envelope, the served meter event and the shadow JSONL line. A0 entries (gate.screen, extraction, prompt-refinement, plan.dor, plan.decompose) are served as ANNOTATION ONLY — `envelope.annotation` carries answers/confidence, no `decision`/`confidence` field, and no caller parameter can request action semantics (raw input is schema-gated).
...
- **Verify:** `node scripts/decision-call.test.mjs` (31 passed, exit 0) · `node scripts/decisions-registry.test.mjs` (26 passed, exit 0) · `node scripts/mcp-extraction.test.mjs && node scripts/mcp-prompt-refinement.test.mjs && node scripts/mcp-protocol.test.mjs && node scripts/mcp-shadow-run.test.mjs && node scripts/mcp-error-scrub.test.mjs` (22/18/14/10/11 passed, exit 0, unchanged) · `node scripts/docs-count-guard.test.mjs` (75/75, exit 0) · `node scripts/graph-validate.test.mjs && node scripts/config-drift.test.mjs` (exit 0) · `node scripts/lint.mjs` (exit 0).
- **Follow-ups:** FOC-397 (graph runner — node entries' questions are built there; plan.ac [G] wiring); FOC-391 (gate.screen wiring — the seed question is advisory and pending); FOC-387 (calibration rows — thresholds stay null until then); FOC-474 / FOC-475 / FOC-449 (remaining registry consumers per the task breakdown).
