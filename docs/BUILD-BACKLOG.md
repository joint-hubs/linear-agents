...
- [x] **T-E0e** exact `sessionId` w manifeście (discovery po `birthtime` w `CLAUDE_CONFIG_DIR`/`~/.claude` projects) + `aggregateRun` exact match zamiast okna `cwd`+`gitBranch`+czas — eliminuje wyciek z aktywnej sesji orkiestratora (1.49M leaked → 6 genuine input tok na realnym `plan.bat`). Fix `cwdToHashName` (`:`→`-`). Pricing `claude-4.8-opus`. Tests 23/23. *(DeepSeek Flash)*
- [x] **T-E7a-polish** `AgentsCostView` — null-safe formatters, nazwy pól wyrównane do API (`costUSD`/`totals.*`), relative time, status pills, sticky header, zebra, empty states, responsywność. *(DeepSeek Flash)*
- [x] **T-E0f** kosztowanie `cache_read` + `cache_creation` (były ignorowane → squad run pokazywał input=6/$0.02 zamiast ctx=158k/$0.31). `costTokens` liczy cacheRead (×`pricing.cacheRead` lub default 0.1×input, konwencja Anthropic) + cacheCreation (×input). `byModel`/`byAgent` eksponują `cacheReadInputTokens`+`cacheCreationInputTokens`. `config/models.json`: cacheRead dla modeli Anthropic (Opus $0.50, Sonnet $0.30 /M). Tests 26/26. *(DeepSeek Flash)*
- [x] **T-E7a-fix2** `AgentsCostView`: kolumna „Context" = fresh+cacheRead+cacheCreation (zamiast mylącego input=6), tooltip z rozbiciem, tabele byModel/byAgent: Fresh/Cache read/Cache write/Out/Cost, wykres po total context, legenda. *(DeepSeek Flash)*
