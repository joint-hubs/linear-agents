...
Follow-ups re-invoke with `claude -p "<message>" --resume <session_id>` (one process per turn; the `session_id` from the `system/init` event is the child's durable identity). The generated `child-settings.json` carries `permissions.deny` for `git push` / `gh pr create` / `gh pr merge` / `gh release create` / `gh api`, so the push gate is harness-enforced; after Mateusz approves, the Supervisor executes the push itself. The spawn script merges the squad's own `settings.json` denies into the generated file itself (union, deduped, deny wins). **Amended 2026-08-26 (FOC-122/FOC-126, as built):** the precedence question this sentence originally hedged is in fact documented — `claude --settings <file>` loads *additional* settings (`claude --help`), so the generated file is merged with the squad's own `CLAUDE_CONFIG_DIR` settings rather than replacing them. That makes the generated file **deny-only**: an `allow` entry could only grant under merge semantics, never remove, and repeating the squad's `hooks` block would risk registering the SessionStart telemetry hook twice and double-counting every child run. The squad's own denies are still copied in, so one file answers "what is this child forbidden to do?" without the reader needing to know the merge rules. `--settings` passed to `supervisor-spawn.mjs` is folded in as one more deny source, never as a substitute — the flag can tighten a child's permissions and cannot loosen them. **Verified empirically 2026-08-26**, because the whole of P9 rests on it: a real child run with `--permission-mode bypassPermissions --settings <generated>` in a sandbox repo with no remote attempted `git push origin main` and the harness refused before git ran — `Permission to use Bash with command git push origin main has been denied.` Deny therefore outranks `bypassPermissions`; the bypass suppresses the prompt, it does not lift the rule. The residual risk below is unchanged: the rule matches on the command prefix, so any indirection still walks past it.
