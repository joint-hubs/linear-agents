...
// targetRepo (optional): the project repo the squad should actually work in,
// resolved from the Linear issue's project → config/projects.json. Defaults
// to rootPath (linear-agents) when no mapping is found — without this, every
// dashboard-triggered launch runs with linear-agents as cwd regardless of
// which repo the task belongs to, so squad meta-artifacts (state, plans,
// briefs) leak into linear-agents instead of the target project.
export function buildLaunchBat(squad, taskId, kickoff, rootPath, targetRepo) {
