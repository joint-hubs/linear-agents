...
// boundary, the tier-2 chat/completions fallback (json_schema + logprobs;
// confidence null without a logprob source), fail-closed exits, the shadow
// JSONL log and its inputs hash. Every HTTP path runs through an injected
...
console.log("\ndecision-call: fallback tier (chat/completions + json_schema + logprobs)");

function fallbackOk(answers, opts = {}) {
  return jsonResponse(fallbackBody(answers, opts));
}

await test("falls back after exhausted 5xx retries and serves tier 2", async () => {
  const tier1Calls = [];
  const impl = async (url) => {
  ...
  eq(envelope.confidence, Math.exp(-0.15), "envelope confidence = exp(mean logprob)");
  eq(envelope.usage.inputTokens, 50, "chat/completions usage normalized");
  eq(tier1Calls.length, 3, "1 + 2 retries before fallback");
});

await test("confidence is null when the fallback returns no logprobs", async () => {
  const impl = async (url) => (url === JEV_ENDPOINT ? statusResponse(500) : fallbackOk({ q0: true }, { logprobs: [] }));
  const envelope = await caller(impl)(NOUL_INPUT);
  eq(envelope.ok, true, "ok");
  eq(envelope.confidence, null, "no logprob source → null, never estimated");
});
...
await test("score and choice questions survive the fallback", async () => {
  ...
  eq(envelope.confidence, Math.exp(-0.15), "logprobs present → exp(mean logprob)");
});

await test("score and choice questions yield null confidence without logprobs", async () => {
  const input = {
    state: "mixed",
    questions: {
      q0: { type: "choice", instructions: "i", criteria: { small: "s", large: "l" } },
...
await test("a non-JSON fallback answer fails closed with unparseable_output", async () => {
  const impl = async (url) => {
    ...
    return jsonResponse({ model: FALLBACK_MODEL, choices: [{ message: { content: "not json" } }] });
  };
  const envelope = await caller(impl)(NOUL_INPUT);
  eq(envelope.ok, false, "ok:false");
...
await test("appends one JSONL shadow line per terminal decision", async () => {
  ...
  try {
    ...
    eq(failed.ok, false, "second call fails (500 everywhere)");

    const lines = readFileSync(join(dir, "decisions.jsonl"), "utf8").trim().split("\n").map((l) => JSON.parse(l));
    eq(lines.length, 2, "one line per decision");
...
await test("fallback serving call carries its own usage meter event", async () => {
  ...
  const records = [];
  const envelope = await caller(impl, { meter: (r) => records.push(r), retries: 1 })(NOUL_INPUT);
  eq(envelope.ok, false, "ok:false");
